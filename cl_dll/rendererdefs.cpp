...
dlinf_s g_dlinf[100];
void DLightFollow(dlinf_s dl)
{
	for (int i = 0; i < 100; i++)
	{
		if (g_dlinf[i].index == 0)
		{
			g_dlinf[i] = dl;
		}
		if (g_dlinf[i].index == dl.index)
			break;
	}
}
...
//===============================
cl_dlight_t *SetupFlashlight(Vector origin, Vector angles, float time, float frametime)
{
	cl_dlight_t* flashlight = gBSPRenderer.CL_AllocDLight(-666);
	flashlight->origin = origin;
	flashlight->radius = 350 * gHUD.m_flFade;
	flashlight->color = Vector(gHUD.m_flFade/2, gHUD.m_flFade / 2, gHUD.m_flFade / 2);
	flashlight->die = time + 0.1;
	flashlight->decay = 5000;
	return flashlight;
#if 0
	pmtrace_t tr;
	Vector fwd, right, up;
